...
def process_media_vllm(image_path=None, video_path=None, text_prompt="Describe the notable events in the provided video.", fps=2.0, total_pixels=12688256, role="Video Timestamping", custom_role_text="", temperature=0.6, top_p=0.95, repetition_penalty=1.05, max_tokens=1024):
    ...
    # Add image(s) if provided - support both single and multiple images
    if image_path is not None:
        # Handle both single image and list of images
        image_paths = image_path if isinstance(image_path, list) else [image_path]
        for img_path in image_paths:
            if img_path is not None:
                # Extract actual path from gradio File object if needed
                if isinstance(img_path, dict):
                    img_path = img_path.get("name") or img_path.get("path") or img_path.get("orig_name")
...
with gr.Blocks(title="Cosmos-Reason1.1", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            image_input = gr.Files(label="Image Input(s) (Optional)", file_types=["image"], file_count="multiple")
            video_input = gr.Video(label="Video Input (Optional)")
            gr.Markdown("**Note:** Both inputs are optional for API usage. You can upload multiple images. Provide either image(s) OR a video (at least one is required for processing).")
            ...
            def update_text_prompt_based_on_role(role):
                ...
                if role == "Video Timestamping":
                    ...
                    default_prompt = "Analyze these images/video and provide insights."
                elif role == "Video Analyzer":
                    default_prompt = "Analyze the video for artifacts and anomalies."
                else:
                    default_prompt = "Describe what you see in these images/video."
    ...
    # Hidden single-file API for backward compatibility
    single_image_input = gr.Image(label="Image (API - single file)", type="filepath", visible=False)
    ...
    process_btn.click(
        ...
        api_name="process_media_multi"
    )
    
    # Hidden single-file API for backward compatibility (original API name)
    api_trigger.click(
        fn=handle_media_processing,
        inputs=[single_image_input, video_input, text_input, fps_input, total_pixels_input, role_selector, custom_role_text, temperature_input, top_p_input, repetition_penalty_input, max_tokens_input],
        outputs=[output, download_file, download_info, process_btn],
    ...
    with gr.Accordion("How it works", open=False):
        ...
        - 🖼️ **Multi-Image Support**: Process single or multiple images with any role
        ...
        - 🔧 **Flexible API**: Call with just image(s), just video, or custom parameters - defaults handle the rest
        - 🔄 **Dual API**: `/process_media` (single image, backward compatible) and `/process_media_multi` (multiple images)
        ...
        ### API Usage:
        **Option 1: Python Client - Multi-Image API**
        ```python
        from gradio_client import Client
        client = Client("http://localhost:8080")
        
        # Multiple images (new API)
        result = client.predict(
            ["/path/to/image1.jpg", "/path/to/image2.jpg"],  # image_input (list)
            api_name="/process_media_multi"
        )
        ...
        # Custom prompt with multiple images
        result = client.predict(
            ["/path/to/image1.jpg", "/path/to/image2.jpg"],  # image_input
            None,                     # video_input
            "Compare these images",   # text_prompt
            api_name="/process_media_multi"
        )
        ```
        ...
        **Option 2: HTTP API (can omit trailing parameters)**
        ...
        curl -X POST "http://localhost:8080/api/process_media_multi" \\
          -H "Content-Type: application/json" \\
          -d '{"data": [["/path/to/image1.jpg", "/path/to/image2.jpg"]]}'
