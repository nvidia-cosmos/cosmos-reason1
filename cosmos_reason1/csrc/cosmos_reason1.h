...
{
    if (result) {
        throw std::runtime_error(std::string("[FT][ERROR] CUDA runtime error: ") + (cudaGetErrorString(result)) + " "
...
namespace cosmos_reason1 {
    ...
    {
        throw std::runtime_error(std::string("[COSMOS][ERROR] ") + info + " Assertion fail: " + file + ":"
                                + std::to_string(line) + " \n");
    }
