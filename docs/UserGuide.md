...
```bash
uv run --extra train wandb login # Then enter your WANDB_API_KEY 
```
...
```shell
uv run --extra train cosmos-rl --config configs/cosmos-reason1/cosmos-reason1-7b-tp2-sft.toml ./tools/dataset/cosmos_sft.py
```
...
```shell
uv run --extra train cosmos-rl --config configs/cosmos-reason1/cosmos-reason1-7b-p-fsdp1-tp2-r-tp2-pp1-grpo.toml tools/dataset/cosmos_grpo.py
```
...
```shell
uv run --extra train tools/eval/inference.py
```
...
```bash
uv run --extra train tools/eval/download_hf_data.py \
    --target data \
    --task benchmark 
```
...
```bash
uv run --extra train tools/eval/calculate_accuracy.py --result_dir results --dataset robovqa
```
